...
class Scheduler:
    ...
    def _can_use_buffer_overlap_scoring(
        ...
        if config.prologue_fusion or config.epilogue_fusion:
            node1_names = node1.get_buffer_names()
            node2_names = node2.get_buffer_names()
            for node in self.nodes:
                if node.is_template():
                    template_deps = OrderedSet(
                        dep.name for dep in node.read_writes.reads
                    )
                    if (node1_names & template_deps) and (node2_names & template_deps):
                        return False
